    ...
    do while not rs.EOF
      ...
      <tr>
        ...
        <td valign="top" align="center"><img onclick="window.open(this.src)"
          onload="imgResizeToFit(this, 160, 98); this.style.display = 'inline';" src="<%= formsServer %>?sessionId=<%= sessionId %>&verb=binaryData&formId=formProfilePets&recordId=<%=  rs("ID") %>&dbFieldBaseName=FOTO&t=<%= timer() %>" /></td>
      </tr>
      <%
      rs.moveNext
    loop
